...
function um_like_article(){
	...
	if($uid){
		...
		if(!isset($meta->dated)||$now_date!=$meta->dated){
			update_um_credit( $uid , $credit , 'add' , 'um_credit' , sprintf( __('参与文章互动，获得%s牛币','um') , $credit ) );
			$new_times = 1;
...
function um_post_source_price($postid){
	...
	if($price[3]==0&&$price[4]==0){
		...
		if($currency==1) $content .= '<em>¥</em><strong>'.sprintf('%0.2f',$price[0]).'</strong><em>(元)</em>'; else $content .= '<em><i class="fa fa-gift"></i></em><strong>'.sprintf('%0.2f',$price[0]).'</strong><em>(牛币)</em>';
		$content .= '</li>';
	}else{ 
		$content .= '<li class="summary-price"><span class="dt">售价 :</span>';
		if($currency==1) $content .= '<em>¥</em><strong><del>'.sprintf('%0.2f',$price[0]).'</del></strong><em>(元)</em>'; else $content .= '<em><i class="fa fa-gift"></i></em><strong><del>'.sprintf('%0.2f',$price[0]).'</del></strong><em>(牛币)</em>';
		if($price[4]!=0){
		...
		if($price[3]!=0){
			...
			if(getUserMemberType()) { 
				if($currency==1) $content .= '<em>¥</em><strong>'.sprintf('%0.2f',$price[1]).'</strong><em>(元)</em>'; else $content .= '<em><i class="fa fa-gift"></i></em><strong>'.sprintf('%0.2f',$price[1]).'</strong><em>(牛币)</em>';
			}else if(is_user_logged_in()){
				$content .= sprintf(__('非 <a href="%1$s" target="_blank" title="开通会员">会员</a> 不能享受该优惠','um'),um_get_user_url('membership'));
			} else {
				if($currency==1) $content .= '<em>¥</em><strong>'.sprintf('%0.2f',$price[6]).'</strong><em>(元)</em>'; else $content .= '<em><i class="fa fa-gift"></i></em><strong>'.sprintf('%0.2f',$price[6]).'</strong><em>(牛币)</em>';$content .= '<a href="javascript:" class="user-login">登录</a> 查看实际享受优惠';
			}
			$content .= '</li>';
		}
	}
...
function um_user_profile_widget(){
	...
	if(um_get_setting('open_uctg')){
		...
		if( count($can_post_cat) ) {
			if( wp_is_mobile() ){
				$mee_url="http://www.diannews.cn/tg";
			}else{
				$mee_url=add_query_arg('action','new',um_get_user_url('post'));
			}
			$shorcut_links[] = array(
				'title' => __('文章投稿','um'),
				'url' => $mee_url
