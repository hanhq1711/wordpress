...
// Affiliate money sum
function get_um_aff_sum_money($uid,$currency='cash'){
	if(!$uid)return;
	global $wpdb;
	$table_name = $wpdb->prefix . 'um_orders';
	$sql = "SELECT SUM(aff_rewards) FROM $table_name WHERE aff_user_id=".$uid." AND order_status=4 AND order_currency='".$currency."'";
