...
function update_um_credit( $user_id , $num , $method='add' , $field='um_credit' , $msg='' ){
	...
	if( $method=='add' ){
		...
		if( $add ){
			add_um_message( $user_id ,  'credit' , current_time('mysql') , ($msg ? $msg : sprintf( __('获得%s牛币','um') , $num )) );
			return $add;
		}
	}
	...
	if($method=='cut'){
		...
		if( $cut ){
			add_um_message( $user_id ,  'credit' , current_time('mysql') , ($msg ? $msg : sprintf( __('消费%s牛币','um') , $num )) );
			return $cut;
		}
	}
	...
	if( $update ){
		add_um_message( $user_id ,  'credit' , current_time('mysql') , ($msg ? $msg : sprintf( __('更新牛币为%s','um') , $num )) );
		return $update;
	}
...
function um_credit_to_void( $user_id , $num, $msg='' ){
	...
	add_um_message( $user_id ,  'credit' , current_time('mysql') , ($msg ? $msg : sprintf( __('消费了%s牛币','um') , $num )) );
	return 0;	
}
function user_register_update_um_credit( $user_id ) {
    if( isset($_COOKIE['um_aff']) && is_numeric($_COOKIE['um_aff']) ){
		...
		if( (count($rec_reg) < $rec_reg_num) &&  !in_array($ua,$rec_reg) ){
			...
			if($reg_credit) update_um_credit( $_COOKIE['um_aff'] , $reg_credit , 'add' , 'um_credit' , sprintf(__('获得注册推广（来自%1$s的注册）奖励%2$s牛币','um') , get_the_author_meta('display_name', $user_id) ,$reg_credit) );
		}
	}
	$credit = um_get_setting('new_reg_credit','50');
	if($credit){
		update_um_credit( $user_id , $credit , 'add' , 'um_credit' , sprintf(__('获得注册奖励%s牛币','um') , $credit) );
	}
}
...
function hook_um_affiliate_check_to_tracker_ajax(){
	if( isset($_COOKIE['um_aff']) && is_numeric($_COOKIE['um_aff']) ){
		...
		if( (count($rec_view) < $rec_view_num) && !in_array($ua,$rec_view) ){
			...
			if($view_credit) update_um_credit( $_COOKIE['um_aff'] , $view_credit , 'add' , 'um_credit' , sprintf(__('获得访问推广奖励%1$s牛币','um') ,$view_credit) );
...
function um_resource_dl_add_credit($uid,$pid,$sid){
	...
	if( !in_array($ua,$dl_users) && $current_user != $uid){
		...
		if($dl_credit) update_um_credit( $uid , $dl_credit , 'add' , 'um_credit' , sprintf(__('你发布的文章《%1$s》中资源被其他用户下载，奖励%2$s牛币','um') ,get_post_field('post_title',$pid),$dl_credit) );
	}
}
function um_comment_add_credit($comment_id, $comment_object){
	...
	if($user_id){
		...
		if( $rec_comment<$rec_comment_num && $rec_comment_credit ){
			update_um_credit( $user_id , $rec_comment_credit , 'add' , 'um_credit' , sprintf(__('获得评论回复奖励%1$s牛币','um') ,$rec_comment_credit) );
...
function um_credit_column_callback( $value, $column_name, $user_id ) {
	...
	if( 'um_credit' == $column_name ){
		...
		$value = sprintf(__('总牛币 %1$s 已消费 %2$s','um'), ($credit+$void), $void );
	}
	return $value;
}
...
function um_whether_signed($user_id){
	if(get_user_meta($user_id,'um_daily_sign',true)){
		...
		if($sign_date != $now_date){
			$sign_anchor = '<a href="javascript:" id="daily_sign" title="签到送牛币">'.__('签到','um').'</a>';
		}else{
