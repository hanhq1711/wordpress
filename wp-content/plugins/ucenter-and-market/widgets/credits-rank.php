...
/* ------------------------------------ */
	function __construct(){
		parent::__construct(false,'Um-牛币排行',array( 'description' => '显示用户牛币排行榜' ,'classname' => 'widget_umcreditsrank'));
	}
	...
	?>
		...
		<?php
			...
			foreach ( $creditsranks as $creditsrank) {
				...
				echo '<li class="umcreditsrank-list"><span class="rank-avatar">'.$avatar.'</span><span class="creditsrank-name"><a href="'.get_author_posts_url($creditsrank->user_id).'" target="_blank" title="'.$user_name.'">'.$user_name.'</a></span><span class="creditsrank-ranking">'.$creditsrank->meta_value.''.__(' 牛币','um').'</span></li>';
			}
			echo '</ul></div>';
		?>
