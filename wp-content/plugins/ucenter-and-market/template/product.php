				...
				<div class="property">
					...
					<div class="summary row">
						<ul>
							...
							<li class="summary-price"><span class="dt"><?php _e('商品售价','um'); ?></span><strong><?php if($currency==1)echo '<em>¥</em>'.sprintf('%0.2f',$discount_arr[0]).'<em>(元)</em>'; else echo '<em><i class="fa fa-gift"></i></em>'.sprintf('%0.2f',$discount_arr[0]).'<em>(牛币)</em>';?></strong></li>
							<?php }else{ ?>
							<li class="summary-price"><span class="dt"><?php _e('商品售价','um'); ?></span><strong><?php if($currency==1)echo '<em>¥</em><del>'.sprintf('%0.2f',$discount_arr[0]).'</del><em>(元)</em>'; else echo '<em><i class="fa fa-gift"></i></em><del>'.sprintf('%0.2f',$discount_arr[0]).'</del><em>(牛币)</em>';?></strong><?php if($discount_arr[4]!=0){?><strong><?php echo '&nbsp;'.sprintf('%0.2f',$discount_arr[2]); ?></strong><span><?php _e('(限时特惠)','um'); ?></span><?php }?></li>
							<?php if($discount_arr[3]!=0){?>
							<li class="summary-price"><span class="dt"><?php _e('会员特惠','um'); ?></span><?php if(getUserMemberType()) { ?><strong><?php if($currency==1)echo '<em>¥</em>'.sprintf('%0.2f',$discount_arr[1]).'<em>(元)</em>'; else echo '<em><i class="fa fa-gift"></i></em>'.sprintf('%0.2f',$discount_arr[1]).'<em>(牛币)</em>';?></strong></li><?php }else if(is_user_logged_in()){echo '<strong>';if($currency==1)echo '<em>¥</em>'.sprintf('%0.2f',$discount_arr[6]).'<em>(元)</em>'; else echo '<em><i class="fa fa-gift"></i></em>'.sprintf('%0.2f',$discount_arr[6]).'<em>(牛币)</em>';echo '</strong>';echo sprintf(__('非<a href="%1$s" target="_blank" title="开通会员">会员</a>不能享受该优惠','um'),um_get_user_url('membership'));} else {_e('<a href="javascript:" class="user-login">登录</a> 查看优惠','um');} ?><?php }?>
							<?php }?>
				...
				<form id="alipayment" name="alipayment" action="<?php echo UM_URI.'alipay/alipayapi.php'; ?>" method="post">
					<div id="pay">
						<div class="part-order">
							<ul>
								<h3><?php _e('订单信息','um'); ?><span><?php _e('（价格单位：','um'); ?><?php if($currency==1)echo '元'; else echo '牛币';?><?php _e('）','um'); ?></span></h3>
						...
						<?php } ?>
							...
							<div id="total-price"><?php _e('总金额：','um'); ?><strong>￥1.00</strong><?php if($currency==1)echo '元'; else echo '牛币';?></div>
						</div>
						<div>
						</div>
